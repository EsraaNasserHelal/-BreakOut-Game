...
HITPADDLE.src = "sounds/paddle_hit.mp3";

const WIN = new Audio();
WIN.src = "sounds/Winnig.mp3";
        ...
        for (let c = 0; c < brick.column; c++) {
            ...
            if (b.status == 1 && COLL_COUNT == 0) {
                if (ball.x + ball.radius > b.x && ball.x - ball.radius < b.x + brick.width
                    ...
                    // SCORE +=SCORE_UNIT;
                    COLL_COUNT++;
                    //  setTimeout(addCrack(crack, b.x,b.y, brick.width, brick.height),100000);
                    tempx = b.x;
                    tempy = b.y;
                    tempWidth = brick.width;
                    temphigtht = brick.height;
                    addCrack();
                }
            }

            else if (b.status == 1 && COLL_COUNT > 0) {
                if (ball.x + ball.radius > b.x && ball.x - ball.radius < b.x + brick.width
                    && ball.y + ball.radius > b.y && ball.y - ball.radius < b.y + brick.height) {
                        // HITBRICK.play();
                    ball.dy = - ball.dy;
...
document.getElementById("Sound").addEventListener("click",function(){
    ...
    document.getElementById("Sound").innerHTML="Un Muted"
